...
def append_row_to_doc(doc_id: str, set_data: dict):
    ...
    # to avoid index drift caused by previously inserted characters.
    for i, text in enumerate(cell_texts):
        if not text:
            continue  # leave blank cells alone

        # Re-fetch the doc to get fresh indices after each insertion
        doc = service.documents().get(documentId=doc_id).execute()
        body_content = doc.get("body", {}).get("content", [])
        for element in body_content:
            if "table" in element:
        ...
        new_row = table_element["table"]["tableRows"][-1]
        cell = new_row["tableCells"][i]
        cell_index = cell["content"][0]["startIndex"]

        service.documents().batchUpdate(
            documentId=doc_id,
            body={"requests": [{"insertText": {"location": {"index": cell_index}, "text": text}}]}
        ).execute()
